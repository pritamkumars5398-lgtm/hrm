...
export default function MonthlyPayrollView({ canManage }: Props) {
  const navigate = useNavigate()
  const { status, month, rows, error, load, goToMonth } = usePayslipStore()
  ...
  const openPayslip = (employeeId: string) => navigate(`/dashboard/payroll/payslip/${employeeId}?month=${month}`)

  return (
    <div className="space-y-6">
      <PayCycleCard canManage={canManage} />

      <Card flush>
        <div className="flex items-center justify-between border-b border-hairline px-4 py-3">
          <div>
            <h2 className="text-[13px] font-semibold text-ink">Monthly payroll</h2>
            <p className="mt-0.5 text-[12px] text-muted">
              {canManage
                ? 'Earnings, deductions and net pay per employee. Click a row to enter or finalize.'
                : 'Earnings, deductions and net pay per employee.'}
            </p>
          </div>

          <div className="flex items-center gap-2 bg-surface border border-hairline/80 p-1 rounded-ctl">
            <button
              type="button"
              onClick={() => void goToMonth(-1)}
              aria-label="Previous month"
              className="inline-flex size-7 items-center justify-center rounded-ctl border border-hairline bg-surface text-muted transition-all hover:border-emerald-500 hover:bg-emerald-50 hover:text-emerald-700 cursor-pointer"
            >
              <ChevronLeft size={13} />
            </button>
            <span className="tnum min-w-28 text-center text-[12.5px] font-bold text-ink">
            ...
            <button
              ...
              className="inline-flex size-7 items-center justify-center rounded-ctl border border-hairline bg-surface text-muted transition-all hover:border-emerald-500 hover:bg-emerald-50 hover:text-emerald-700 disabled:pointer-events-none disabled:opacity-40 cursor-pointer"
            >
              ...
              <tbody>
                {isLoading ? (
                  ...
                  <tr>
                    <td colSpan={3} className="px-4 py-16 text-center">
                      ...
                      <p className="mx-auto mt-1 max-w-xs text-[13px] text-muted">
                        Add someone in Employee Management first.
                      </p>
                    </td>
                  </tr>
                ) : (
                  rows.map((row) => (
                    <tr
                      key={row.employeeId}
                      onClick={() => openPayslip(row.employeeId)}
                      tabIndex={0}
                      onKeyDown={(e) => {
                        if (e.key === 'Enter' || e.key === ' ') {
                          e.preventDefault()
                          openPayslip(row.employeeId)
                        }
                      }}
                    ...
                    >
                      ...
                      <td className="tnum px-4 py-3.5 text-right text-[13.5px] font-semibold text-ink">
                        {row.hasSalaryStructure ? (
                          ...
                          <span className="text-[12px] font-medium text-muted">No salary set</span>
                        )}
                      </td>
                    </tr>
                  ))
