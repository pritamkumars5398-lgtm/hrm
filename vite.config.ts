...
/**
 * Local dev only: handles POST /api/send-invite with nodemailer so the email
 * flow works in `npm run dev` the same way it does on Vercel serverless.
 * Receives the full env map so non-VITE_ vars are accessible (Vite only puts
 * VITE_-prefixed vars into process.env by default).
 */
function localApiPlugin(env: Record<string, string>): Plugin {
  return {
    name: 'local-api-send-invite',
